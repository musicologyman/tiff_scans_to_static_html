...
class _coconut(object):
    ...
    Ellipsis, NotImplemented, NotImplementedError, Exception, AttributeError, ImportError, IndexError, KeyError, NameError, TypeError, ValueError, StopIteration, RuntimeError, all, any, bool, bytes, callable, classmethod, dict, enumerate, filter, float, frozenset, getattr, hasattr, hash, id, int, isinstance, issubclass, iter, len, list, locals, map, min, max, next, object, property, range, reversed, set, setattr, slice, str, sum, super, tuple, type, vars, zip, repr, print, bytearray = Ellipsis, NotImplemented, NotImplementedError, Exception, AttributeError, ImportError, IndexError, KeyError, NameError, TypeError, ValueError, StopIteration, RuntimeError, all, any, bool, bytes, callable, classmethod, dict, enumerate, filter, float, frozenset, getattr, hasattr, hash, id, int, isinstance, issubclass, iter, len, list, locals, map, min, max, next, object, property, range, reversed, set, setattr, slice, str, sum, staticmethod(super), tuple, type, vars, zip, staticmethod(repr), staticmethod(print), bytearray
...
ARABIC_TO_ROMAN = dict(((arabic), (roman)) for arabic, roman in zip(range(1, len(ROMAN_NUMERALS) + 1), ROMAN_NUMERALS))
...
def get_toc_entry(file_name):
    return _coconut_tail_call((TOC_TEMPLATE.substitute), {'file_name': file_name, 'page_number': extract_number(file_name)})


web_pages_dir = ((Path('~/Documents/scanned_documents/2023_02_24/jpg')).expanduser())


toc_entries = (reduce)(_coconut.operator.add, (list)((map)(lambda s: s + '<br>', (map)(_coconut.functools.partial(reduce, _coconut.operator.add), (groupsof)(10, (map)(get_toc_entry, (filter)(PAGE_NUMBER_RE.search, (sorted)((map)(lambda p: p.name, (web_pages_dir).glob('page*.html'))))))))))

(Path(web_pages_dir / 'index.html').write_text)(((string.Template)(Path('index_template.html').read_text())).substitute({'entries': toc_entries}))
